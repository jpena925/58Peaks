...
function Planning({ allPeaks }) {
	...
	const [selectedMountain, setSlectedMountain] = useState({
		...
		mainImage:
			"https://www.14ers.com/photos/mtelbert/peakphotos/medium/200506_Elbert02.jpg?lu=20210623",
		latitude: 39.1178,
		longitude: -106.4452,
		campgrounds: [
	...
	useEffect(() => {
		fetch(
			`http://api.weatherapi.com/v1/current.json?key=${weatherKey}&q=${selectedMountain.latitude},${selectedMountain.longitude}&aqi=no`
		)
			.then((res) => res.json())
			.then((data) => {
	...
	const weather = currentWeather ? (
		<div className="weather">
			...
			<p>
				...
				<img src={currentWeather.condition.icon} id="weather-icon" />
			</p>
			<p>
				<strong>Temperature:</strong> {currentWeather.temp_f}°
			</p>
			<p>
				<strong>Feels like:</strong> {currentWeather.feelslike_f}°
			</p>
			<p>
				<strong>Wind:</strong> {currentWeather.wind_mph}mph
			</p>
			<p>
	...
	const campgrounds = selectedMountain.campgrounds.map((camp) => {
		...
		return (
			<li key={camp} className="campsite-li">
				<a
					className="campsite-link"
					target="_blank"
					href={`https://www.google.com/search?q=${searchVersion}`}>
					{camp}
				</a>
			...
			<div id="plan-hike-split">
				<div className="mountain-select">
					<form
						id="mountain-form"
						onSubmit={(e) =>
							handleSelectedMountain(e, e.target.querySelector("input").value)
						}>
						...
						<datalist id="mountains">
							{allPeaks.map((peak) => (
								<option value={peak.name} id={peak.id}>
									{peak.name}
								</option>
							))}
						</datalist>
